...
def _resolve_local_cuda_rank() -> int:
    visible_gpus = torch.cuda.device_count()
    if visible_gpus <= 0:
        return 0

    # Prefer rank-based mapping to avoid env var mismatch in Ray workers.
    if torch.distributed.is_initialized():
        return torch.distributed.get_rank() % visible_gpus

    local_rank = int(os.environ.get("RAY_LOCAL_RANK", os.environ.get("LOCAL_RANK", "0")))
...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        local_rank = _resolve_local_cuda_rank()
    ...
    def update_actor(self, data: DataProto):
        ...
        # --- required meta for dp_actor.update_policy ---
        data.meta_info.setdefault("temperature", float(self.config.rollout.temperature))
        data.meta_info.setdefault("micro_batch_size", int(self.config.actor.ppo_micro_batch_size))
        data.meta_info.setdefault("max_token_len", int(self.config.actor.ppo_max_token_len_per_gpu))
        data.meta_info.setdefault("use_dynamic_bsz", bool(self.config.actor.use_dynamic_bsz))
    ...
    def compute_log_prob(self, data: DataProto) -> DataProto:
        ...
        # ---- meta_info: always ensure required keys exist ----
        data.meta_info.setdefault('eos_token_id', self.tokenizer.eos_token_id)
        data.meta_info.setdefault('pad_token_id', self.tokenizer.pad_token_id)

        # required by dp_actor.compute_log_prob
        data.meta_info.setdefault("temperature", float(self.config.rollout.temperature))
        data.meta_info.setdefault("micro_batch_size", int(self.config.rollout.log_prob_micro_batch_size))
        data.meta_info.setdefault("max_token_len", int(self.config.rollout.log_prob_max_token_len_per_gpu))
        data.meta_info.setdefault("use_dynamic_bsz", bool(self.config.rollout.log_prob_use_dynamic_bsz))
