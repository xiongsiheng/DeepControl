...
def load_hf_weights(actor_weights: Dict, vllm_model: nn.Module):
    ...
    normalized_actor_weights = {}
    for name, value in actor_weights.items():
        new_name = name
        for prefix in ("_fsdp_wrapped_module.", "module._fsdp_wrapped_module.", "module."):
            if new_name.startswith(prefix):
                new_name = new_name[len(prefix):]
                break
        normalized_actor_weights[new_name] = value

    with set_default_torch_dtype(next(vllm_model.parameters()).dtype):  # TODO
        if vllm_model.config.tie_word_embeddings and "lm_head.weight" in normalized_actor_weights.keys():
            del normalized_actor_weights["lm_head.weight"]
        vllm_model.load_weights(normalized_actor_weights.items())
