...
class HFLoader(BaseModelLoader):
    ...
    def _normalize_weight_name(name: str) -> str:
        for prefix in ("_fsdp_wrapped_module.", "module._fsdp_wrapped_module.", "module."):
            if name.startswith(prefix):
                return name[len(prefix):]
        return name

    @staticmethod
    def _is_fsdp_flat_param(name: str) -> bool:
        return name == "_flat_param" or name.endswith("._flat_param") or "._flat_param." in name

    def _get_weights_iterator(self, actor_model: Union[PreTrainedModel, Dict]):
        if isinstance(actor_model, Dict):
            normalized = {}
            for k, v in actor_model.items():
                nk = self._normalize_weight_name(k)
                if self._is_fsdp_flat_param(nk):
                    continue
                normalized[nk] = v
            return normalized.items()
        elif isinstance(actor_model, nn.Module):
            source_module = getattr(actor_model, "_fsdp_wrapped_module", actor_model)
            normalized = {
                self._normalize_weight_name(k): v
                for k, v in source_module.named_parameters(remove_duplicate=False)
                if not self._is_fsdp_flat_param(self._normalize_weight_name(k))
            }
            return normalized.items()
        else:
