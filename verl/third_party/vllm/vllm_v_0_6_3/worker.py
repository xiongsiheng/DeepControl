...
class Worker(Worker):
    ...
    def init_device(self) -> None:
        if self.device_config.device.type == "cuda":
            ...
            local_rank = int(os.getenv("RAY_LOCAL_RANK", os.getenv("LOCAL_RANK", "0")))
            visible_gpus = torch.cuda.device_count()
            if visible_gpus > 0:
                if local_rank < 0 or local_rank >= visible_gpus:
                    # Fallback to rank-based mapping for Ray worker env mismatch.
                    local_rank = self.rank % visible_gpus if self.rank >= 0 else 0
